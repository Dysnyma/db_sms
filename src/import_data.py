...
def main():
    ...
    try:
        ...
        n = import_csv(
            cursor,
            f"{DATA_DIR}/course_offering.csv",
        ...
        with open(f"{DATA_DIR}/enrollment.csv", "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                # 检查重复
                cursor.execute(
                    "SELECT 1 FROM enrollment WHERE offering_id=%s AND student_id=%s",
                    [oid, sid],
                )
                if cursor.fetchone():
                    continue
                ...
                if score_val is not None:
                    cursor.execute(
                        "INSERT INTO enrollment (offering_id, student_id, score) VALUES (%s, %s, %s)",
                        [oid, sid, float(score_val)],
                    )
                else:
                    cursor.execute(
                        "INSERT INTO enrollment (offering_id, student_id) VALUES (%s, %s)",
                        [oid, sid],
                    )
                n += 1
        log(f"选课 + 成绩：{n} 条")
